...
echo '<div class="user_details2 flex sb">';
...
foreach ($vaccins as $vaccin) {
    ...
    if(mb_strlen($documents) > 0){
        ...
        foreach ($docsArray as $document) {
            if(file_exists('../upload/user_'.$currentUser['id'].'/info_'.$vaccin['id'].'/'.$document)){
                if(mb_strlen($documentInfo) > 0){
                    $documentInfo .= '<br>';
                }
                $documentInfo .= '<a target="_blank" href="../upload/user_'.$currentUser['id'].'/info_'.$vaccin['id'].'/'.$document.'"><i class="fas fa-file"></i> '.str_replace('.pdf', '', $document).'</a> <a class = "gestion_supprimer" href="?delete_file='.$document.'&info_id='.$vaccin['id'].'&id='.$userid.'">Supprimer</a>';
            }
        }
        echo $documentInfo;
    }
